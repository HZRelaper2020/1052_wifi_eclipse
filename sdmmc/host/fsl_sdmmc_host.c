...
#include "semphr.h"
extern SemaphoreHandle_t sdio_transfer_finished_semaphore;
...
//    if (g_sdio_start_transfer_flag && status ==kStatus_USDHC_SendCommandSuccess ){
//    host_rtos_set_semaphore(&sdio_transfer_finished_semaphore,1);
//    static int num =0;
//    num++;
////    DbgConsole_Printf("give %d %d ",num,status);
//    }
    ...
    {
//    	if (0x402c4000 == (unsigned int)base && handle->data && handle->data->rxData && handle->data->txData)
